...
class EstatesConnector @Inject() (http: HttpClientV2, config: FrontendAppConfig) {
  ...
  def register(payload: Declaration)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[DeclarationResponse] = {
    ...
    http
      ...
      .execute[DeclarationResponse](DeclarationResponse.httpReads, ec)
  }
  ...
  def getPersonalRepName()(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[PersonalRepName] = {
    ...
    for {
      individual   <- http
                        .get(url"$getPersonalRepIndUrl")
                        .execute[Option[PersonalRepName]](PersonalRepName.personalRepIndividualNameReads, ec)
      organisation <- http
                        .get(url"$getPersonalRepOrgUrl")
                        .execute[Option[PersonalRepName]](PersonalRepName.personalRepOrganisationNameReads, ec)
    } yield (individual, organisation) match {
